...
export default function Billing() {
  ...
  const handleGenerateInvoice = async (forcedStatus?: string) => {
                  ...
                  {paymentMethod === 'Online' && (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {['UPI', 'Card', 'Net Banking', 'Wallet', 'EMI'].map((sub) => (
                          <button
                            key={sub}
                            type="button"
            ...
            <div className="p-8 -mt-6 bg-white rounded-t-[2.5rem] flex-1 space-y-8">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  ...
                  <div className="font-black text-slate-900 leading-tight">{customerInfo.name || 'Acme Corp'}</div>
                  <div className="text-[10px] font-bold text-slate-400">{customerInfo.email || 'billing@acme.com'}</div>
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
            ...
            >
              ...
              <div ref={invoiceRef} className={cn(
                "space-y-12 bg-white p-8",
                invoiceTheme === 'Classic' ? "font-serif border-[12px] border-slate-100" : "font-sans"
              )}>
                 <div className="flex justify-between items-start">
                   <div className="space-y-4 max-w-sm">
                      <div className={cn(
                        "text-3xl font-black tracking-tight uppercase leading-none",
                        invoiceTheme === 'Modern' ? "text-slate-900" : "text-black"
                      )}>
                       {profile?.businessName || 'BILLCORE'}
                      </div>
                      <div className="space-y-1">
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-relaxed">
                          {invoiceSettings.address || profile?.location || 'Official Business Address Not Set'}
                        </p>
                        <div className="flex items-center gap-2">
                           <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">GSTIN:</span>
                           <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">
                             {invoiceSettings.gstin || profile?.gstin || 'NA'}
                           </span>
                        </div>
                      </div>
                   </div>
                   <div className="text-right">
                      <div className={cn(
                        ...
                        invoiceTheme === 'Modern' ? "text-blue-600" : "text-slate-400"
                      )}>Electronic Voucher</div>
                      <div className="text-[10px] font-black text-slate-900 font-mono italic opacity-50 space-x-4">
                         <span>SEC-ID: {viewingInvoice ? viewingInvoice.invoiceNumber : Date.now().toString().slice(-8)}</span>
                      </div>
                   </div>
                 </div>
                 
                 {/* Identity Matrix Registry */}
                 <div className="grid grid-cols-2 gap-12 bg-slate-50/50 p-10 rounded-[2.5rem] border border-slate-100 relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600/5 rotate-45 -mr-16 -mt-16"></div>
                    
                    <div className="space-y-6 relative z-10">
                       <div className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] mb-4 flex items-center gap-2">
                          <span className="w-8 h-px bg-slate-200"></span>
                          Issuer Credentials
                       </div>
                       <div className="space-y-2">
                          <p className="text-xs font-bold text-slate-600 leading-relaxed max-w-xs italic">
                            {viewingInvoice ? viewingInvoice.customerAddress : (invoiceSettings.address || profile?.location || 'Official Register details not configured.')}
                          </p>
                          <div className="flex items-center gap-3 pt-2">
                             <span className="text-[9px] font-black text-slate-400 uppercase">GSTIN Protocol:</span>
                             <span className="text-[10px] font-black text-slate-900">{viewingInvoice ? viewingInvoice.gstin : (invoiceSettings.gstin || profile?.gstin || 'Unrecorded')}</span>
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="space-y-6 relative z-10">
                       ...
                       <div className="space-y-2">
                          <div className="text-lg font-black text-slate-900 tracking-tight">{viewingInvoice ? viewingInvoice.customerName : (customerInfo.name || 'Anonymous Entity')}</div>
                          <div className="space-y-1">
                             <div className="text-xs font-bold text-slate-500">{viewingInvoice ? viewingInvoice.customerPhone : customerInfo.phone}</div>
                             <div className="text-xs font-bold text-slate-400 italic">{viewingInvoice ? viewingInvoice.customerEmail : (customerInfo.email || 'Email missing')}</div>
                          </div>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className={cn(
                   ...
                   invoiceTheme === 'Modern' ? "rounded-2xl border border-slate-200" : 
                   invoiceTheme === 'Classic' ? "border-2 border-black" : 
                   "rounded-none border border-slate-400"
                 )}>
                   <table className="w-full text-left">
                      <thead>
                         <tr className={cn(
                           "text-[10px] font-black uppercase tracking-[0.1em]",
                           invoiceTheme === 'Modern' ? "bg-slate-50 border-b border-slate-200 text-slate-400" : 
                           invoiceTheme === 'Classic' ? "bg-black text-white" : 
                           "bg-slate-100 border-b border-slate-400 text-slate-600"
                         )}>
                            <th className="px-6 py-4">Index</th>
                            <th className="px-6 py-4">Item & Descriptor</th>
                            <th className="px-6 py-4 text-center">Qty</th>
                            <th className="px-6 py-4 text-right">Unit Rate</th>
                            <th className="px-6 py-4 text-right">Tax (%)</th>
                            <th className="px-6 py-4 text-right">Aggregated</th>
                         </tr>
                      </thead>
                     <tbody className={cn(
                       "divide-y",
                       invoiceTheme === 'Modern' ? "divide-slate-100" : 
                       invoiceTheme === 'Classic' ? "divide-black" : 
                       "divide-slate-200"
                     )}>
                        {(viewingInvoice ? viewingInvoice.items : selectedItems).map((item: any, idx: number) => (
                          <tr key={idx} className="text-xs hover:bg-slate-50/50 transition-colors">
                             <td className="px-6 py-5 font-mono font-bold text-slate-400">{(idx + 1).toString().padStart(2, '0')}</td>
                             <td className="px-6 py-5">
                                <div className="font-black text-slate-900 mb-0.5">{item.name}</div>
                             ...
                             <td className="px-6 py-5 text-right text-slate-600 font-bold">₹{item.price.toLocaleString()}</td>
                             <td className="px-6 py-5 text-right text-slate-400 font-mono italic">{item.gstRate}%</td>
                             <td className="px-6 py-5 text-right font-black text-slate-900 border-l border-slate-50">₹{(item.price * item.qty).toLocaleString()}</td>
                          </tr>
                        ))}
                     </tbody>
                  </table>
                ...
                <div className="flex justify-between items-start gap-12 pt-4">
                   <div className="flex-1 space-y-4">
                      <div className={cn(
                        "p-6",
                        invoiceTheme === 'Modern' ? "rounded-3xl border-2 border-dashed border-slate-100 bg-slate-50/30" : 
                        invoiceTheme === 'Classic' ? "border-t border-black bg-white" : 
                        "rounded-xl border border-slate-200 bg-slate-50/50"
                      )}>
                        <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-3">Compliance Guarantee</h4>
                        <p className={cn(
                          "text-[10px] leading-relaxed font-bold uppercase tracking-tighter whitespace-pre-line",
                          invoiceTheme === 'Modern' ? "text-slate-400" : "text-black"
                        )}>
                          {viewingInvoice ? viewingInvoice.terms : (invoiceSettings.terms || profile?.invoiceTerms || "This is a computer-generated invoice and doesn't require a physical signature. All transactions are reported to central systems.")}
                        </p>
                      </div>

                      {(viewingInvoice ? viewingInvoice.paymentMethod !== 'Cash' : paymentMethod !== 'Cash') && (
                        <div className={cn(
                          "p-6",
                          invoiceTheme === 'Modern' ? "rounded-3xl bg-slate-50/50 border border-slate-100" : "border-t border-black"
                        )}>
                           <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-4">Institutional Settlement Details</h4>
                           <div className="grid grid-cols-2 gap-8">
                             <div className="space-y-3">
                                <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Beneficiary Institution</div>
                                <div className="text-xs font-black text-slate-900 uppercase tracking-tight">{viewingInvoice ? viewingInvoice.bankName : (invoiceSettings.bankName || profile?.bankName || 'General Bank of Commerce')}</div>
                                <div className="grid grid-cols-2 gap-4 pt-1">
                                  <div>
                                     <div className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-1">Account No.</div>
                                     <div className="text-[10px] font-black text-slate-800 font-mono tracking-widest">{viewingInvoice ? viewingInvoice.accountNo : (invoiceSettings.accountNo || profile?.accountNumber || 'XXXXXXXXXXXX')}</div>
                                  </div>
                                  <div>
                                     <div className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-1">IFSC Protocol</div>
                                     <div className="text-[10px] font-black text-slate-800 font-mono tracking-widest">{viewingInvoice ? viewingInvoice.ifsc : (invoiceSettings.ifsc || profile?.ifscCode || 'XXXX0000XXX')}</div>
                                  </div>
                                </div>
                             </div>
                             {invoiceSettings.upiId && (
                               <div className="space-y-4 border-l border-slate-100 pl-8 flex items-start gap-6">
                                  <div className="space-y-2 flex-1">
                                    <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Digital Gateway (UPI)</div>
                                    <div className="text-sm font-black text-blue-600 font-mono tracking-tight">{invoiceSettings.upiId}</div>
                                    <div className="text-[8px] text-slate-400 font-bold uppercase italic mt-1">Scan & Pay utilizing any authorized UPI processor.</div>
                                  </div>
                                  {(paymentMethod === 'Online' && onlineMethod === 'UPI') || (invoiceSettings.upiId && (paymentMethod as string).includes('UPI')) ? (
                                    <div className="p-2 bg-white rounded-xl shadow-sm border border-slate-100 hover:shadow-xl transition-all group relative">
                                      <div className="absolute inset-0 bg-blue-600/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl"></div>
                                      <QRCodeSVG 
                                        value={`upi://pay?pa=${invoiceSettings.upiId}&pn=${encodeURIComponent(profile?.businessName || 'Merchant')}&am=${viewingInvoice ? viewingInvoice.totalAmount : calculateTotal()}&cu=INR`}
                                        size={70}
                                        level="H"
                      ...
                      {invoiceSettings.notes && (
                        <div className={cn(
                          "p-6",
                          invoiceTheme === 'Modern' ? "rounded-3xl bg-blue-50/20 border border-blue-100/50" : 
                          "rounded-xl border border-slate-200 border-dashed"
                        )}>
                          <h4 className="text-[10px] font-black text-blue-900 uppercase tracking-widest mb-1 italic">Administrative Notes</h4>
                          <p className="text-[11px] text-slate-600 font-medium leading-relaxed">{invoiceSettings.notes}</p>
                        </div>
                      )}
                   </div>
                   <div className={cn(
                     "w-80 p-8 space-y-4 shadow-2xl relative overflow-hidden",
                     invoiceTheme === 'Modern' ? "bg-slate-900 rounded-3xl text-white" : 
                     invoiceTheme === 'Classic' ? "bg-white text-black border-4 border-black border-double" : 
                     "bg-slate-100 rounded-xl text-slate-900 border border-slate-400"
                   )}>
                      {invoiceTheme === 'Modern' && <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600/10 blur-3xl -mr-16 -mt-16"></div>}
                      <div className="flex justify-between text-[11px] font-black uppercase tracking-widest opacity-40">
                        <span>Net Value</span>
                        <span className="font-mono">₹{(viewingInvoice ? viewingInvoice.subtotal : calculateSubtotal()).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-[11px] font-black uppercase tracking-widest opacity-40">
                        <span>Tax ({gstType})</span>
                        <span className="font-mono">₹{(viewingInvoice ? viewingInvoice.totalGst : calculateTax()).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-[11px] font-black uppercase tracking-widest text-rose-500">
                        <span>Discounts</span>
                        <span className="font-mono">-₹{(viewingInvoice ? viewingInvoice.totalDiscount : calculateTotalDiscounts()).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-widest italic gap-4">
                        <span className="opacity-40">Payment Mode</span>
                        <select 
                          className={cn(
                            "bg-transparent outline-none cursor-pointer border-b border-transparent hover:border-blue-500/30 transition-colors uppercase font-black text-right",
                            invoiceTheme === 'Modern' ? "text-blue-400" : "text-black"
                          )}
                          value={viewingInvoice ? viewingInvoice.paymentMethod : paymentMethod}
                          onChange={(e: any) => updateStoredPaymentMethod(e.target.value)}
                        >
                      ...
                      <div className={cn(
                        "h-px my-2",
                        invoiceTheme === 'Modern' ? "bg-white/10" : "bg-black/20"
                      )}></div>
                      <div className="flex justify-between text-3xl font-black tracking-tighter pt-2">
                        <span>{(viewingInvoice?.theme || invoiceTheme) === 'Classic' ? 'TOTAL' : 'Final Amt.'}</span>
                        <span className={cn(
                          "font-mono",
                          (viewingInvoice?.theme || invoiceTheme) === 'Modern' ? "text-blue-400" : "text-black"
                        )}>{formatCurrency(viewingInvoice ? viewingInvoice.totalAmount : calculateTotal())}</span>
                      </div>
                      {invoiceSettings.footer && (
                        <div className={cn(
                          "pt-4 text-[9px] font-bold text-center opacity-40 uppercase tracking-widest leading-relaxed italic",
                          invoiceTheme === 'Modern' ? "border-t border-white/5" : "border-t border-black/5"
                        )}>
                          {invoiceSettings.footer}
                        </div>
                      )}
                ...
                <div className="pt-12 text-center space-y-2 border-t border-slate-100">
                   <div className="text-[12px] font-black text-slate-900 uppercase tracking-[0.4em]">Thank You</div>
                   <div className="text-[9px] text-slate-300 font-bold uppercase tracking-widest">System Validation Confirmed • SecuraLedger Protocol v4.2</div>
                </div>
              </div>
